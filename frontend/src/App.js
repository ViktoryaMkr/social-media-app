...
function App() {
  ...
  return (
    <>
      <div className="App">
        <Routes>
          <Route path="/" element={isLoggedIn ? <Navigate to='/home' /> : <Navigate to='/login' replace/>} />
          <Route path="/home" element={isLoggedIn ? <HomePage /> : <Navigate to='/login' replace />} />
          <Route path="/profile/:id" element={isLoggedIn ? <ProfilePage /> : <Navigate to='/login' replace/>} />
